...
class Pos extends Component
{
    ...
    public function mount()
    {
        ...
        if ($this->orderID) {
            ...
            $order = Order::with(['table', 'extraCharges', 'customer'])->find($this->orderID);
            ...
            if ($this->orderDetail->status === 'draft' && $this->orderDetail->items->count() > 0) {
                ...
                foreach ($this->orderDetail->kot as $kot) {
                    ...
                    foreach ($kot->items->where('status', '!=', 'cancelled') as $item) {
                        ...
                        // Set price context before calculating amounts
                        if ($this->orderTypeId) {
                            $item->menuItem->setPriceContext($this->orderTypeId, $normalizedDeliveryAppId);
                            if ($item->menuItemVariation) {
                                $item->menuItemVariation->setPriceContext($this->orderTypeId, $normalizedDeliveryAppId);
                            }
                            // Set context on modifiers too
    ...
    public function saveOrder($action, $secondAction = null, $thirdAction = null)
    {
        ...
        if ((!$this->tableOrderID && !$this->orderID) || ($this->tableOrderID && !$this->tableOrder->activeOrder)) {
            ...
            if ($wasDraft && $status !== 'draft') {
                ...
                // Updating a non-draft order - delete items to recreate
                $order->items()->delete();
            }
            $order->taxes()->delete();
        }
