            ...
            <div class="flex items-center justify-start">
                ...
                <a href="{{ route('dashboard') }}" class="flex ltr:ml-2 rtl:mr-2 items-center app-logo">
                    ...
                    @if (restaurant()->show_logo_text)
                        <span
                            class="self-center text-xl font-semibold sm:text-xl whitespace-nowrap dark:text-white hidden lg:block ltr:mr-2 rtl:ml-2">{{ Str::limit(restaurant()->name, 10) }}</span>
                    @endif
                </a>

                @if (!request()->routeIs('pos.*'))
                    <button id="toggle-sidebar" type="button"
                        class="lg:inline-flex items-center p-2 text-sm text-gray-500 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600 mx-2 hidden">
                        <!-- Menu expand icon (shows when sidebar is collapsed) -->
                        <svg id="toggle-sidebar-open" class="hidden w-6 h-6 transition-transform duration-200"
                            xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M13 5l7 7-7 7M5 5l7 7-7 7" />
                        </svg>
                        <!-- Menu collapse icon (shows when sidebar is expanded) -->
                        <svg id="toggle-sidebar-close" class=" w-6 h-6 transition-transform duration-200"
                            xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex gap-2">
                    ...
                    @if (in_array('Reservation', restaurant_modules()) &&
                            user_can('Show Reservation') &&
                            restaurant()->hide_new_reservations == 0 &&
                            in_array('Table Reservation', restaurant_modules()))
                        @livewire('dashboard.todayReservations')
                    @endif

                    @if (in_array('Waiter Request', restaurant_modules()) &&
                            user_can('Manage Waiter Request') &&
                            restaurant()->hide_new_waiter_request == 0)
            ...
            <div class="flex items-center gap-4 w-fit justify-end">
                ...
                @if (restaurant()->package->package_type == \App\Enums\PackageType::DEFAULT)
                    <a href="{{ route('pricing.plan') }}" wire:navigate class="inline-flex">
                        <x-secondary-button
                            class="inline-flex items-center gap-2 shadow-md text-skin-base dark:text-skin-base hover:origin-center group px-2 sm:px-3">
                            <svg class="w-5 h-5 text-current group-hover:scale-110 duration-500" width="24"
                ...
                @elseif (restaurant()->package->package_type == \App\Enums\PackageType::TRIAL)
                    <a href="{{ route('pricing.plan') }}" wire:navigate class=" hidden lg:inline-flex">
                        <x-secondary-button class="px-2 sm:px-3">
                            @php
                                $daysLeftInTrial = floor(
                                    now(timezone())->diffInDays(
                                        \Carbon\Carbon::parse(restaurant()->trial_ends_at)->addDays(1),
                                    ),
                                );
                            @endphp
                            <span
                                class="text-xs sm:text-sm">{{ $daysLeftInTrial > 0 ? $daysLeftInTrial . ' ' . __('modules.package.daysLeftTrial') : __('modules.package.trialExpired') }}</span>
                        </x-secondary-button>
                    </a>
                @endif
